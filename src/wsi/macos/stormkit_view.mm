...
- (void)handleMouseDown:(NSEvent *)event {
    requester->mouse_down_event(mouse_button([event buttonNumber]),
...
- (void)handleMouseUp:(NSEvent *)event {
    requester->mouse_up_event(mouse_button([event buttonNumber]),
                              event.locationInWindow.x,
                              event.locationInWindow.y);
}
...
- (void)handleMouseMove:(NSEvent *)event {
    if (is_mouse_inside) {
        ...
        requester->mouse_move_event(point.x, point.y);
    }
}
...
- (void)keyDown:(NSEvent *)event {
    ...
    if ([string length] > 0) key = localized_key([string characterAtIndex:0]);

    if (key == std::numeric_limits<std::uint8_t>::max()) key = non_localized_key(event.keyCode);

    requester->key_down_event(key, [string characterAtIndex:0]);
}
...
- (void)keyUp:(NSEvent *)event {
    ...
    requester->key_up_event(key, [string characterAtIndex:0]);
}
