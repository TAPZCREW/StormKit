...
namespace stormkit::wsi::linux::wayland {
    ...
    auto WindowImpl::create(std::string title, const math::Extent2<u32>& extent, WindowStyle style)
      -> void {
        ...
        // create surface and server side decoration if supported
        ...
        const auto stride = usize { extent.width * sizeof(u32) };
        const auto size   = usize { stride * extent.height };

        // create surface pixel buffer and attach it to surface
        SHMBuffer::create(size, std::format("StormKit::{}::PixelBuffer", title))
          .transform(bind_front(&DeferInit<SHMBuffer>::construct<SHMBuffer&&>, &m_pixel_buffer))
          .transform_error(monadic::assert());

        stdr::fill(m_pixel_buffer.get(), Byte { 0 });

        m_shm_pool = wl::ShmPool {
            std::in_place,
            globals.shm,
            narrow<i32>(std::bit_cast<uptr>(m_pixel_buffer->native_handle())),
            narrow<i32>(size)
        };

        m_shm_buffer = wl::Buffer {
            wl_shm_pool_create_buffer(m_shm_pool,
                                      0,
                                      narrow<i32>(extent.width),
                                      narrow<i32>(extent.height),
                                      narrow<i32>(stride),
                                      WL_SHM_FORMAT_XRGB8888)
        };
        wl_buffer_add_listener(m_shm_buffer, &wl::g_buffer_listener, &m_shm_buffer);

        wl_surface_commit(m_surface);
        wl_surface_attach(m_surface, m_shm_buffer, 0, 0);
        wl_surface_damage(m_surface, 0, 0, as<i32>(m_extent.width), as<i32>(m_extent.height));
...
//     }
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto WindowImpl::create_surface() noexcept -> void {
        // xdg_wm_base_add_listener(globals.xdg_shell.get(), &stormkit_shell_listener, nullptr);

        // m_xdg_surface.reset(xdg_wm_base_get_xdg_surface(globals.xdg_shell.get(),
        // m_surface.get()));
    ...
    //    m_shm_pool.reset(wl_shm_create_pool(globals.shm.get(), as<i32>(fd),
    //    as<i32>(buffer_size)));
    ...
    /////////////////////////////////////
    /////////////////////////////////////
    auto WindowImpl::update_keymap(std::string_view keymap) noexcept -> void {
    ...
    //    auto* window = static_cast<WindowImpl*>(data);
    //    window
    //      ->keyboard_modifiers(keyboard, serial, mods_depressed, mods_latcher, mods_locked,
    //      group);
    // }
