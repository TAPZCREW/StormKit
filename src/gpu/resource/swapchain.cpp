...
namespace stormkit::gpu {
    namespace {
        ...
        auto choose_swap_extent(const VkSurfaceCapabilitiesKHR& capabilities,
                                const math::ExtentU&            extent) noexcept -> VkExtent2D {
            ...
            auto actual_extent   = as<VkExtent2D>(extent);
        ...
        /////////////////////////////////////
        /////////////////////////////////////
        auto choose_image_count(const VkSurfaceCapabilitiesKHR& capabilities) noexcept -> UInt32 {
    ...
    auto SwapChain::do_init(const Device&        device,
        ...
        return vk_call<VkSurfaceCapabilitiesKHR>(vkGetPhysicalDeviceSurfaceCapabilitiesKHR,
          ...
          .transform([this, &device](auto&& vk_images) noexcept {
              m_image_count = as<UInt32>(std::ranges::size(vk_images));
              m_images      = vk_images
      ...
      -> Expected<NextImage> {
        ...
        return vk_call<VkResult>(m_vk_device_table->vkAcquireNextImageKHR,
          ...
          .transform([&id](auto&& result) {
              return NextImage { .result = from_vkflags<Result>(result), .id = id };
          })
          .transform_error(monadic::from_vkflags<Result>());
    }
} // namespace stormkit::gpu
