...
namespace {
    ...
    constexpr auto SWAPCHAIN_EXTENSIONS = std::array { VK_KHR_SWAPCHAIN_EXTENSION_NAME };
...
namespace stormkit::gpu {
    ...
    auto Device::do_init(const Instance& instance, const Info& info) noexcept -> Expected<void> {
        ...
        auto handle = VkDevice { nullptr };
        auto result = vk_call<VkDevice>(vkCreateDevice,
                                        m_physical_device->native_handle(),
                                        &create_info,
                                        nullptr)
                        .transform(core::monadic::set(handle))
                        .and_then([this, &allocator_create_info, handle] mutable noexcept {
                        ...
                        .and_then([this, &raster_queue] noexcept {
                            ...
                            return set_object_name(*this,
                                                   std::format("StormKit:Device ({})",
                                                               m_physical_device->info()
                                                                 .device_name));
                        });
        std::println("AAAAAAAAAAAAAAAAAAAAAAaaa {} {}", m_vk_handle.value(), handle);
        m_vk_handle = std::move(handle);
        return result;
    }
