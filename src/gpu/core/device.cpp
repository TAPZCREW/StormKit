...
namespace {
    ...
    VkResult vmaImportVulkanFunctionsFromVolk(const VmaAllocatorCreateInfo* pAllocatorCreateInfo,
        ...
        stormkit::expects(pAllocatorCreateInfo->instance != nullptr);
        stormkit::expects(pAllocatorCreateInfo->device != nullptr);
...
namespace stormkit::gpu {
    ...
    auto Device::do_init(const Instance& instance, const Info& info) noexcept -> Expected<void> {
        ...
        const auto raster_queue = [&queue_families]() -> Queue_ {
            ...
            return {
                .id    = as<UInt32>(std::distance(std::ranges::cbegin(queue_families), it)),
                .count = it->count,
                .flags = it->flags,
            };
        }();
        ...
        const auto  enabled_features = [&capabilities] noexcept {
            auto out = VkPhysicalDeviceFeatures {};
            zero_bytes(out);
        ...
        const auto acceleration_feature = [] static noexcept {
            auto out = VkPhysicalDeviceAccelerationStructureFeaturesKHR {};
            zero_bytes(out);
        ...
        const auto rt_pipeline_feature = [&acceleration_feature] noexcept {
            auto out  = VkPhysicalDeviceRayTracingPipelineFeaturesKHR {};
        ...
        const auto create_info = VkDeviceCreateInfo {
            ...
            .enabledExtensionCount   = as<UInt32>(std::ranges::size(extensions)),
        ...
        auto allocator_create_info = VmaAllocatorCreateInfo {
            ...
            .vulkanApiVersion               = vk_make_version<Int32>(1, 4, 0),
