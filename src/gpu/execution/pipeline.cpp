...
namespace stormkit::gpu {
    ...
    auto Pipeline::do_init(const PipelineLayout&            layout,
      ...
      -> Expected<void> {
        ...
        // const auto vk_pipeline_cache = [&pipeline_cache] -> const Vkraii::PipelineCache& {
        //     if (pipeline_cache) return toRaiiVkHandle(*pipeline_cache);
        //     return nullptr;
        // };

        const auto vk_pipeline_cache = core::
          either(pipeline_cache,
                 core::monadic::map(monadic::to_vk(), core::monadic::init<VkPipelineCache>()),
                 core::monadic::init<VkPipelineCache>(nullptr));
