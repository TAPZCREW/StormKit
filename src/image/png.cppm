...
namespace stormkit::image::details {
    ...
    auto load_png(byte_view data) noexcept -> std::expected<image::Image, image::Image::Error> {
        ...
        png_get_IHDR(png_ptr, info_ptr, &extent.width, &extent.height, &bit_depth, &color_type, nullptr, nullptr, nullptr);
        ...
        auto row_pointers = dynarray<byte*> { extent.height, nullptr };

        auto buff_pos = std::data(image_memory);

        for (auto i : range(extent.height)) row_pointers[i] = &buff_pos[row_bytes * i];

        png_read_image(png_ptr, std::bit_cast<png_bytepp>(std::data(row_pointers)));
