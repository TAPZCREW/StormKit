...
namespace stormkit::gpu {
    ...
    Swapchain::Swapchain(Tag,
        ...
        const auto  capabilities    = physical_device.native_handle()
                                    .getSurfaceCapabilitiesKHR(*surface.native_handle());
        const auto formats = physical_device.native_handle().getSurfaceFormatsKHR(*(surface.native_handle()));
    ...
    auto Swapchain::acquire_next_image(std::chrono::nanoseconds wait,
      ...
      -> Expected<std::pair<gpu::Result, UInt32>> {
        auto&& [result, index] = m_vk_swapchain
                                   ->acquire_next_image(wait.count(), to_vkhandle(image_available));
