package("vulkan-memory-allocator-hpp", function()
    ...
    on_install("windows|x86", "windows|x64", "linux", "macosx", "mingw", "android", "iphoneos", function(package)
        if not package:config("modules") then
            ...
            io.writefile(
                "xmake.lua",
                format([[--add_requires("vulkan-memory-allocator",
                                                            "vulkan-header >= v1.3.297")
                                                 target("vulkan-memory-allocator-hpp")
                                                     set_kind("moduleonly") set_languages("c++20")
                                                         add_headerfiles("include/(**.hpp)")
                                                             add_includedirs("include") add_files(
                                                                 "src/*.cppm",
                                                                 { public = true })
                                                                 add_packages(
    ...
    on_test(
        function(package)
            assert(package:check_cxxsnippets({
                test = [[void test() { int version = VMA_VULKAN_VERSION; }]],
            }, {
                includes = "vk_mem_alloc.hpp",
