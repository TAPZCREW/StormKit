package("libxkbcommon_")
set_homepage("https://xkbcommon.org/")
set_description("keymap handling library for toolkits and window systems")
set_license("MIT")

add_urls(
    "https://github.com/xkbcommon/libxkbcommon/archive/refs/tags/xkbcommon-$(version).tar.gz",
...
if is_plat("linux") then add_extsources("apt::libxkbcommon-dev") end
...
on_load(function(package)
    ...
    if package:config("wayland") then
        ...
        if package:is_plat("linux") then package:add("extsources", "pacman::libxkbcommon") end
    end
end)
...
on_test(function(package) assert(package:has_cfuncs("xkb_context_new", { includes = "xkbcommon/xkbcommon.h" })) end)
