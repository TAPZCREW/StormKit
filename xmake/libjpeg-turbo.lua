if is_plat("windows") then
    ...
    on_load(function(package)
        if package:is_built() then package:add("deps", "cmake", "nasm") end
    end)
    ...
    on_install("windows", "linux", "macosx", "android", "mingw", function(package)
        ...
        local runtimes = package:runtimes() or package:config("vs_runtime")
        if package:is_plat("windows") and runtimes and runtimes:startswith("MD") then
            table.insert(configs, "-DWITH_CRT_DLL=ON")
        end
        if package:is_plat("mingw") then table.insert(configs, "-DCMAKE_SYSTEM_PROCESSOR=" .. package:arch()) end
    ...
    on_test(function(package) assert(package:has_cfuncs("tjSaveImage", { includes = "turbojpeg.h" })) end)
end
