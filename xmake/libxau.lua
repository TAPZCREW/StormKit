...
if is_plat("linux") then add_extsources("apt::libxau-dev", "pacman::libxau", "pkgconfig::xau") end

if is_plat("macosx", "linux", "bsd") then add_deps("pkg-config", "util-macros", "xorgproto") end
...
on_install("macosx", "linux", "bsd", function(package)
    ...
    if package:config("pic") then table.insert(configs, "--with-pic") end
    import("package.tools.autoconf").install(package, configs)
end)

on_test(function(package) assert(package:has_ctypes("Xauth", { includes = "X11/Xauth.h" })) end)
